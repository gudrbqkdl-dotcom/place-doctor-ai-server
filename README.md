...
- 방문자리뷰 수는 네이버 공식 검색 API에서 자동 조회되지 않으므로, 이번 화면에서는 리뷰 추적 기능을 제외했습니다.
...
- 전체 블로그 순위: 대표키워드 단독 검색 기준 상위 블로그를 보여주고, 업체 관련 글은 별도 표시합니다.
- AI 블로그 원고: 분석 후 추천 키워드를 최대 3개까지 선택하면 AI가 제목과 원고를 자동으로 생성합니다.
- 썸네일 PNG 다운로드: 생성된 원고의 제목, 업체명, 선택 키워드를 사용해 1:1 정사각형 블로그 썸네일 이미지를 만들고 `.png`로 다운로드할 수 있습니다. 별도 이미지 생성 API 없이 브라우저에서 생성됩니다.
...
주의: 현재 화면은 분석 속도를 빠르게 하기 위해 `/api/analyze`에서 AI 원고를 기다리지 않습니다. 점수와 순위가 먼저 뜬 뒤, 추천 키워드를 최대 3개까지 선택해야 `/api/draft`가 호출되어 AI가 제목과 원고를 생성합니다.
...
분석 결과를 바탕으로 AI 블로그 원고만 따로 생성합니다. 화면에서는 추천 키워드를 먼저 보여주고, 사용자가 최대 3개까지 선택한 뒤 `블로그 발행 원고작성` 버튼을 누르면 호출됩니다. 제목은 따로 선택하지 않고 AI가 선택 키워드에 맞춰 새로 만듭니다.
...
```json
{
  ...
  "recommendedKeywords": {},
  "prompt": "AI 블로그 작성 프롬프트",
  "selectedKeywords": ["동해헬스장", "동해PT", "동해피트니스"]
}
```
...
- 블로그 현재 순위는 `업체명+키워드`가 아니라 사용자가 입력한 대표키워드 단독 검색 결과에서만 계산합니다. 예를 들어 업체명이 `트리트라움 피트니스`, 키워드가 `동해헬스장`이면 네이버 블로그 API 검색어는 오직 `동해헬스장`입니다.
- 서버는 대표키워드 단독 검색 결과 최대 100개 안에서 업체명이 포함된 블로그 글을 자동으로 전부 찾습니다. `트리트라움 동해헬스장`, `트리트라움 피트니스 동해헬스장`, `트리트라움` 같은 블로그 검색은 현재순위 계산에 사용하지 않습니다.
- 서버 응답에서 `keywordBlogResults`는 대표키워드 전체 상위 블로그 목록이고, `ownBlogResults`는 그 목록 안에서 업체명이 포함된 내 글 확인 결과입니다. 화면의 `전체 블로그 순위` 영역은 `keywordBlogResults`를 우선 표시하고 업체 관련 글을 함께 표시합니다.
- 화면에는 트리트라움 관련 블로그가 발견된 모든 순위를 `1위, 4위, 12위`처럼 표시합니다.
...
- 업체명 칸에 `트리트라움 동해헬스장`처럼 키워드가 같이 들어가도 서버가 키워드를 빼고 `트리트라움` 같은 실제 브랜드명으로만 결과를 확인합니다.
...
- 입력한 업체명, 키워드, 업종, 자동 추천 키워드, 상위 블로그 목록을 참고해 추천 키워드를 보여주고, 사용자가 최대 3개까지 선택하면 AI가 제목과 네이버 블로그 발행용 완성 원고를 생성합니다.
