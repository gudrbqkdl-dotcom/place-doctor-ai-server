...
- 블로그 현재 순위는 `업체명+키워드`가 아니라 사용자가 입력한 대표키워드 단독 검색 결과에서 계산합니다. 예를 들어 업체명이 `트리트라움 피트니스`, 키워드가 `동해헬스장`이면 순위 계산 검색어는 오직 `동해헬스장`입니다.
- 서버는 대표키워드 단독 검색 결과 최대 100개 안에서 업체명이 포함된 블로그 글을 자동으로 전부 찾습니다.
- 화면에는 트리트라움 관련 블로그가 발견된 모든 순위를 `1위, 4위, 12위`처럼 표시합니다.
- `트리트라움 피트니스 동해헬스장` 같은 업체명+키워드 검색은 보조 확인용으로만 사용하며 현재순위에는 넣지 않습니다.
