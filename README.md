...
- AI 블로그 원고: 분석 후 추천 제목과 추천 키워드를 먼저 보여주고, 사용자가 두 가지를 선택하면 네이버 블로그에 붙여넣을 완성 원고를 생성합니다.
- MD 다운로드: 생성된 블로그 원고를 `.md` 파일로 바로 다운로드할 수 있습니다.
- 썸네일 PNG 다운로드: 생성된 원고의 제목, 업체명, 대표키워드를 사용해 웹에서 바로 블로그 썸네일 이미지를 만들고 `.png`로 다운로드할 수 있습니다. 별도 이미지 생성 API 없이 브라우저에서 생성됩니다.
...
주의: 현재 화면은 분석 속도를 빠르게 하기 위해 `/api/analyze`에서 AI 원고를 기다리지 않습니다. 점수와 순위가 먼저 뜬 뒤, 추천 제목과 추천 키워드를 선택해야 `/api/draft`가 호출되어 AI 원고를 생성합니다.
...
분석 결과를 바탕으로 AI 블로그 원고만 따로 생성합니다. 화면에서는 추천 제목과 추천 키워드를 먼저 보여주고, 사용자가 두 가지를 선택한 뒤 `블로그 발행 원고작성` 버튼을 누르면 호출됩니다.
...
```json
{
  ...
  "recommendedKeywords": {},
  "prompt": "AI 블로그 작성 프롬프트",
  "selectedTitle": "동해헬스장 등록 전 꼭 확인해야 하는 7가지",
  "selectedKeyword": "동해헬스장"
}
```
...
- 서버 응답에서 `keywordBlogResults`는 대표키워드 전체 상위 블로그 목록이고, `ownBlogResults`는 그 목록 안에서 업체명이 포함된 내 글 확인 결과입니다. 화면의 `공식 API 상위 블로그` 영역은 `keywordBlogResults`를 우선 표시합니다.
...
- 입력한 업체명, 키워드, 업종, 자동 추천 키워드, 상위 블로그 목록을 참고해 추천 제목과 추천 키워드를 보여주고, 사용자가 선택한 조합으로 네이버 블로그 발행용 완성 원고를 생성합니다.
